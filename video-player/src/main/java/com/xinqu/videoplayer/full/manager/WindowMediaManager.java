...
import com.xinqu.videoplayer.full.WindowMediaInterface;
import com.xinqu.videoplayer.full.WindowMediaSystem;
import com.xinqu.videoplayer.full.WindowResizeTextureView;

/**
 * 这个类用来和jzvd互相调用，当jzvd需要调用Media的时候调用这个类，当MediaPlayer有回调的时候，通过这个类回调
 * Created by Nathen on 2017/11/18.
 */
public class WindowMediaManager implements TextureView.SurfaceTextureListener {

    public static final String TAG = "JiaoZiVideoPlayer";
    public static final int HANDLER_PREPARE = 0;
    public static final int HANDLER_RELEASE = 2;
    public static WindowResizeTextureView textureView;
    public static SurfaceTexture savedSurfaceTexture;
    public static Surface surface;
    public static WindowMediaManager mediaManager;
    public int positionInList = -1;
    public WindowMediaInterface mXinQuMediaInterface;
    ...
    public WindowMediaManager() {
        mMediaHandlerThread = new HandlerThread(TAG);
        mMediaHandlerThread.start();
        mMediaHandler = new MediaHandler(mMediaHandlerThread.getLooper());
        mainThreadHandler = new Handler();
        if (mXinQuMediaInterface == null)
            mXinQuMediaInterface = new WindowMediaSystem();
    }

    public synchronized static WindowMediaManager instance() {
        synchronized(WindowMediaManager.class){
            if (mediaManager == null) {
                mediaManager = new WindowMediaManager();
            }
        }
        return mediaManager;
    }
    ...
    public static void setLoop(boolean loop) {
        if(null!=instance().mXinQuMediaInterface){
            instance().mXinQuMediaInterface.setLoop(loop);
        }
    }
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture, int i, int i1) {
        Log.i(TAG, "onSurfaceTextureAvailable [" + WindowVideoPlayerManager.getCurrentJzvd().hashCode() + "] ");
