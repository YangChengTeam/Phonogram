...
public class CategoryDetailFragment extends BaseFragment {
    ...
    public void init() {
        ...
        RxView.clicks(ivBackList).throttleFirst(200, TimeUnit.MILLISECONDS).subscribe(new Action1<Void>() {
            @Override
    ...
    private void getData() {
        new CategoryDetailEngine(getActivity()).getCategoryDetail(categoryInfo.getId()).subscribe(new Action1<ResultInfo<CategoryDetailInfo>>() {
            @Override
            public void call(ResultInfo<CategoryDetailInfo> categoryDetailInfoResultInfo) {
    ...
    private void playVideo() {
        RxView.clicks(videoPlayer.thumbImageView).throttleFirst(1000, TimeUnit.MICROSECONDS).subscribe(new Action1<Void>() {
            @Override
            public void call(Void aVoid) {
                judgeVip();
            }
        });
        RxView.clicks(videoPlayer.startButton).throttleFirst(1000, TimeUnit.MICROSECONDS).subscribe(new Action1<Void>() {
            @Override
            public void call(Void aVoid) {
                judgeVip();
            }
